...
| **服务** | 启动/停止/重启、实时日志流输出、端口检测（CHECK_PORT 只读探测通道，端口语义由主进程裁决，renderer 只展示） | 新增 |
...
- **管理员**：管理员用户名/密码/令牌有效期（expire_min）
...
1. 用户打开配置页 → Electron Main 读取 config.yaml + .env → **掩码敏感字段（掩码值 = 空串）** → 通过 IPC 返回给 Renderer
2. 用户修改后点「保存」→ Renderer 通过 IPC 发送新配置 → Electron Main **掩码空串在业务校验前回填磁盘现值** → 分别写入两个文件
...
- 后台监听地址必须是 IP 或 localhost（isIP 校验，ADR-0010 网络隔离）
...
| preload 单文件打包 | esbuild bundle | 沙箱 preload 的 require 白名单约束，多文件 split 会挂 |
